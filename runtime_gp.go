...
func strategyFor(p *Page, pol cadence.Policy, id string, ctx cadence.RequestContext) cadence.Interpretation {
	...
	if pol == nil {
		switch any(p.regionKind(id)).(type) {
		case cadence.Stateful:
			s = cadence.Live{}
		case cadence.Plain:
			s = cadence.Deferred{Where: cadence.Server{}, On: cadence.OnLoad{}}
		default:
...
func wireTag(i cadence.Interpretation) (string, string) {
	switch __gp_m1 := any(i).(type) {
	case cadence.Inline:
		return "eager", "onload"
	case cadence.AfterPaint:
		on := __gp_m1.On
		return "deferred", triggerName(on)
	case cadence.LiveTransport:
		return "live", ""
	case cadence.ClientCompute:
		return "eager", "onload"
	default:
		panic("goplus: impossible enum value in match")
	}
